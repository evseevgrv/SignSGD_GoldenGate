...
class GaLoreProjector:
    ...
    def update_proj(self, full_rank_grad, step=None):
        if self.proj_type == "svd" or (self.proj_type == "power_iteration" and self.ortho_matrix is None):
            self.ortho_matrix = self._get_orthogonal_matrix(full_rank_grad, self.rank, type=self.proj_side)
        elif self.proj_type == "power_iteration":
            self.ortho_matrix = self._get_orthogonal_matrix_power_it(full_rank_grad, self.ortho_matrix, type=self.proj_side)
