...
def check_args_torchrun_main(args):
    assert args.rank == 0 or args.density == 0.0, "One should specify either `rank` of `density`."
    assert not (args.rank == 0 and "apollo_mini" in args.optimizer.lower()), "Please specify rank for apollo_mini."
    if args.optimizer.lower() == "frugal":
        args.optimizer = "block_adamw"
    if (args.proj_embeds or args.proj_logits or args.proj_norms) and ("galore" in args.optimizer.lower() or "coord" in args.optimizer.lower()):
    ...
    args.mp_policy_param = getattr(torch, args.mp_policy_param)
    args.mp_policy_reduce = getattr(torch, args.mp_policy_reduce)
    args.mp_policy_buffer = getattr(torch, args.mp_policy_buffer)

    if args.dtype == getattr(torch, "bfloat16"):
        args.mp_policy_param = getattr(torch, "bfloat16")
        args.mp_policy_reduce = getattr(torch, "bfloat16")
        args.mp_policy_buffer = getattr(torch, "bfloat16")
    ...
    if args.compile and args.fsdp:
        args.use_orig_params = True

    if args.optimizer.lower() == "ldadam":
        assert args.beta1 == 0.908
...
def get_pretraining_name_and_tags(args):
    ...
    args.wandb_name += f"opt-{args.optimizer}-dtype-{args.dtype}-amp-{int(args.amp)}-bs-{args.total_batch_size}-sch-{args.scheduler}-warmup-{args.warmup_steps}-wd-{args.weight_decay}-lr-{args.lr}"
    ...
    # TODO adam and other
    if "sgd" in args.optimizer and args.optimizer != "aid_sign_sgd":
        args.wandb_name += f"-lr_scale-{args.proj_params_lr_scale}-nesterov-{int(args.nesterov)}-momentum-{args.momentum}-sign-{int(args.sgd_sign_update)}-sign_norm-{int(args.sign_norm)}-normalized-{int(args.normalized)}"
        args.wandb_tags += ['nesterov' if args.reset_statistics else 'no-nesterov', f"momentum_{args.momentum}", 'sign' if args.sgd_sign_update else 'no-sign']
    elif args.optimizer in ["aid_sign_sgd", "aid_with_adam"]:
        args.wandb_name += f"-l_inf-{args.l_inf}-d_0-{args.d_0}-lb-{args.lower_bound}-clamp-{args.clamp_level}-gap-{args.update_gap}"
        args.wandb_tags += [f"l_inf_{args.l_inf}"]
        if args.d_0 is not None:
            args.wandb_tags += [f"d_0_{args.d_0}", "option1"]
        if args.lower_bound is not None:
            args.wandb_tags += [f"option2"]
        if args.optimizer == "aid_with_adam":
            args.wandb_name += f"-emb-{int(args.proj_embeds)}-logit-{int(args.proj_logits)}-norm-{int(args.proj_norms)}"
    elif args.optimizer.lower() == "adamw_delayed":
        args.wandb_name += f"-emb-{int(args.proj_embeds)}-logit-{int(args.proj_logits)}-norm-{int(args.proj_norms)}"
        args.wandb_name += f"-delay-{args.delay_start_step}-taylor-{int(args.grad_taylor_approx)}"
        if args.grad_taylor_approx:
            args.wandb_name += f"-{args.taylor_linear_coef}"
    ...
    elif "block" in args.optimizer or "sgd_with_adam" in args.optimizer:
        if "block" in args.optimizer:
