...
def get_optimizer(param_groups, args, model=None):
    ...
    elif optimizer_name == "adamw_delayed":
        from adamw_delayed import AdamWDelayed
        optimizer = AdamWDelayed(param_groups, delay_start_step=args.delay_start_step, grad_taylor_approx=args.grad_taylor_approx, taylor_linear_coef=args.taylor_linear_coef, betas=(args.beta1, args.beta2), lr=args.lr, weight_decay=args.weight_decay, eps=args.eps)#, foreach=False, fused=False)
    elif optimizer_name == "sgd_with_adam":
        optimizer = SgdWithAdam(
            param_groups, 
            lr = args.lr * args.proj_params_lr_scale * args.inactive_lr_scale,
    ...
    elif optimizer_name == "sign_sgd":
        optimizer = SgdWithSign(
            param_groups, 
            lr = args.lr * args.proj_params_lr_scale * args.inactive_lr_scale,
            momentum = args.momentum,
            dampening = args.dampening,
    ...
    elif optimizer_name == "aid_sign_sgd":
    ...
    elif optimizer_name == "aid_with_adam":
        optimizer = AIDWithAdam(
            param_groups, 
            L_inf=args.l_inf, # 100.0
            lower_bound=args.lower_bound, # 0.0
            d_0=args.d_0,
            weight_decay = args.weight_decay,
            clamp_level=args.clamp_level, # I use clamp_level=lr
            update_gap=args.update_gap,
