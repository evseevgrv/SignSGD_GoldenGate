...
class DoWG(Optimizer):
    ...
    def __init__(self, params, eps=1e-4, weight_decay=0.0):
        defaults = dict(eps=eps, weight_decay=weight_decay)
        self.eps = eps
    ...
    def step(self, closure=None):
        ...
        with torch.no_grad():
            ...
            for idx, group in enumerate(self.param_groups):
                ...
                grad_sq_norm += torch.stack([(p.grad ** 2).sum() for p in group["params"] if p.grad is not None]).sum()
...
class CDoWG(Optimizer):
    ...
    def __init__(self, params, eps=1e-8, weight_decay=0.0):
        defaults = dict(eps=eps, weight_decay=weight_decay)
